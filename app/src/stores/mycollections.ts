...
export const useCollectionsStore = defineStore('mycollectionsStore', () => {
	...
	async function hydrate() {
		console.log("Hidrate")
		const response = await api.get<any>(`/mycollections`);
	...
	async function upsertCollection(collection: string, values: DeepPartial<Collection & { fields: Field[] }>) {
		...
		try {
			if (existing) {
				...
				const createdCollectionResponse = await api.post<{ data: CollectionRaw }>('/mycollections', rawValues);
	...
	async function updateCollection(collection: string, updates: DeepPartial<Collection>) {
		try {
			await api.patch(`/mycollections/${collection}`, updates);
			await hydrate();
	...
	async function deleteCollection(collection: string) {
		...
		try {
			await api.delete(`/mycollections/${collection}`);
